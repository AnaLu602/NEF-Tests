...
# Return Codes:
# 0 - Successful test
# 1 - Test Failed due to errors in getting serving cell information
...
def validate_report(report, nef_supi):
    ...
    for request in report:
        if request["endpoint"] == f"/test/api/v1/UEs/{nef_supi}/serving_cell":
            if request["method"] != "GET":
                errors.append("Wrong method used.")
                break
            elif request["nef_response_code"] not in [200, 409]:
                errors.append(f"Unable to get serving cell information due to: {request['nef_response_message']}")
                break
    return errors
